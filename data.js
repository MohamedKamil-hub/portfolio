...
const PORTFOLIO = {
  ...
  statusLines: [
    ...
    { label: "role",     value: "Systems & Infrastructure Technician", highlight: false },
  ...
  experience: [
    {
      role:     { en: "Systems & Infrastructure Technician",           es: "Tecnico de Sistemas e Infraestructura" },
      ...
      tags:     ["Linux", "Ubuntu", "Debian", "Windows 11", "Cisco", "Networking", "Troubleshooting", "ITSM", "Hardening", "2FA", "MDM", "Hardware", "Bash"],
      techStack: { en: "Linux (Debian/Ubuntu), Windows 11, Velocloud SD-WAN, Zscaler, Fortinet, Microsoft Intune, YubiKey 2FA, Cisco switching, ITSM tooling", es: "Linux (Debian/Ubuntu), Windows 11, Velocloud SD-WAN, Zscaler, Fortinet, Microsoft Intune, YubiKey 2FA, switching Cisco, herramientas ITSM" },
      bullets: {
        en: [
          "On-site systems support for an Amazon-CEVA Logistics deployment in Ontigola - full maquetacion cycles across a 100+ workstation production-line network, replacing aged endpoints with updated hardware.",
          "Deployed 20-30 workstations per shift across 4 device classes: Debian/Ubuntu thin clients, RF barcode scanners, thermal label printers, and industrial weight scales - end-to-end provisioning, network configuration and production-line integration validation.",
          "Reduced per-station configuration time by 75% (40 minutes -> 10 minutes) through process improvements and reusable provisioning checklists adopted across the team.",
          "Hardened endpoint baseline: enforced locale standardisation, automated language-pack deployment via apt, validated hardware-token 2FA setup, and rotated legacy default credentials across 100+ devices.",
          "Resolved a recurring scanner read-error class affecting ~15% of RF units by identifying a locale/keymap mismatch and codifying the fix into a 4-step remediation procedure.",
          "Calibrated 2 thermal printer models for label-feed accuracy and integration with thin-client print spoolers - documented per-model media settings and feed-mode configuration.",
          "Diagnosed and fixed application-layer scale connectivity failures using TCP port probes for pre-flight validation - cut intermittent shipping-app errors during peak production.",
          "User administration at warehouse scale: account provisioning, static IP assignment, WiFi onboarding, driver installation (WiFi adapters, printers) across a multi-layer enterprise security stack (SD-WAN, secure web gateway, NGFW, MDM).",
          "ITSM ticket triage under SLA across 5+ daily incidents: classified as remote-resolvable vs on-site dispatch, reducing unnecessary engineer travel.",
          "Post-deployment phase: incident response, connectivity troubleshooting and asset labelling; authored an internal infrastructure runbook to accelerate onboarding for new team members.",
          "Worked with enterprise network infrastructure (routers, switches, switch stacking, Windows 11 imaging via USB) and physical cabling in a live production environment.",
        ],
        es: [
          "Soporte de sistemas on-site en el despliegue Amazon-CEVA Logistics en Ontigola - ciclos completos de maquetacion sobre una red de produccion de 100+ workstations, sustituyendo endpoints antiguos por hardware actualizado.",
          "Despliegue de 20-30 workstations por turno en 4 categorias de dispositivo: thin clients Debian/Ubuntu, lectores RF, impresoras termicas de etiquetas y basculas industriales - provisioning de extremo a extremo, configuracion de red y validacion de integracion con linea de produccion.",
          "Reduccion del tiempo de configuracion por estacion en un 75% (40 minutos -> 10 minutos) mediante mejoras de proceso y listas de verificacion reutilizables adoptadas por el equipo.",
          "Hardening del baseline de endpoints: estandarizacion de locales, despliegue automatizado de paquetes de idioma via apt, validacion de 2FA con token hardware y rotacion de credenciales por defecto legacy en 100+ dispositivos.",
          "Resolucion de una clase de errores de lectura recurrente que afectaba al ~15% de los lectores RF identificando un mismatch de locale/teclado y codificando la solucion en un procedimiento de remediacion de 4 pasos.",
          "Calibracion de 2 modelos de impresora termica para precision de avance de etiqueta e integracion con spoolers de impresion de thin clients - documente ajustes por modelo y modos de avance.",
          "Diagnostico y resolucion de fallos de conectividad a nivel de aplicacion en basculas usando sondas TCP de puerto como pre-flight - reduje errores intermitentes de la app de shipping en pico de produccion.",
          "Administracion de usuarios a escala de almacen: aprovisionamiento de cuentas, asignacion de IP estatica, onboarding WiFi e instalacion de drivers (adaptadores WiFi, impresoras) sobre un stack de seguridad empresarial multicapa (SD-WAN, secure web gateway, NGFW, MDM).",
          "Triaje de tickets ITSM bajo SLA en 5+ incidencias diarias: clasificacion como resolucion remota vs presencial, reduciendo desplazamientos innecesarios de ingenieros.",
          "Fase post-despliegue: respuesta a incidencias, troubleshooting de conectividad y etiquetado de activos; autoria de un runbook interno de infraestructura para acelerar el onboarding de nuevos miembros del equipo.",
          "Trabajo con infraestructura de red empresarial (routers, switches, switch stacking, imaging Windows 11 via USB) y cableado fisico en entorno de produccion real.",
        ]
      }
    },
    {
    ...
    {
      role:     { en: "IT Technician (Internship)",      es: "Tecnico IT (Practicas)" },
      org:      "Colegio San Bernardo (Fundacion)",
      period:   { en: "Mar 2025 - Apr 2025",             es: "Mar 2025 - Abr 2025" },
      modality: { en: "On-site - Madrid - Internship",   es: "Presencial - Madrid - Practicas" },
...
Escribir una herramienta de monitorizacion en Go me obligo a aprender goroutines, channels y lectura de recursos a nivel syscall. El sistema de archivos \`/proc\` es sorprendentemente rico.
  ...
  i18n: {
    en: {
      ...
      "hero.role":       "Systems & Infrastructure Technician",
      "hero.bio":        "Systems and infrastructure work on Linux. Built a production WireGuard management panel from scratch during my last internship - FastAPI, Python and Bash on a 512 MB VPS. 40 simultaneous clients, under 50 ms latency. Currently on-site at Randstad Digital on the Amazon-CEVA Logistics deployment in Ontigola, where my team cut per-station provisioning time by 75%. Open to roles where I can keep working on real Linux infrastructure.",
    ...
    es: {
      ...
      "hero.home":       "whoami",
      "hero.greeting":   "whoami",
      "hero.role":       "Tecnico de Sistemas e Infraestructura",
      "hero.bio":        "Sistemas e infraestructura sobre Linux. Construi un panel de gestion WireGuard en produccion desde cero durante mis ultimas practicas - FastAPI, Python y Bash en un VPS de 512 MB. 40 clientes simultaneos, menos de 50 ms de latencia. Actualmente on-site en Randstad Digital, en el despliegue Amazon-CEVA Logistics en Ontigola, donde mi equipo redujo el tiempo de provisioning por estacion en un 75%. Abierto a roles donde seguir trabajando con infraestructura Linux real.",
